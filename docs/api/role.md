...
- **Method:** GET  
- **Path:** `/api/roles`  
- **Query:** `search`, `status`, `from_date`, `to_date`, `sort_by` (id | name | guard_name | status | created_at | updated_at), `sort_order`, `limit`.  
- **Response:** Paginated collection (RoleResource), có quan hệ team, permissions.
